...
def talk_c(content):
    Haruka = skn.win32com.client.Dispatch("SAPI.SpVoice")
    dog = skn.win32com.client.Dispatch("SAPI.SpObjectTokenCategory")
    dog.SetID(r"HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Speech_OneCore\Voices", False)

    v = [t for t in dog.EnumerateTokens() if t.GetAttribute("Name") == "Microsoft Haruka"]
    if v:
        oldv = Haruka.Voice
        Haruka.Voice = v[0]
        Haruka.Speak(content)
        Haruka.Voice = oldv
...
while True:
    try:
        ...
        if "安い" in text:
            ...
            browser.get(link + price)#ページ読み込み
            talk_c(price_only+ "円以上の商品です。ごらんください")
            talk_c(rooptext)
            rooptext = ""
            startflag = False
            ...
            browser.get(link + price)#ページ読み込み
            talk_c(price_only+ "円以下の商品です。ごらんください")
            talk_c(rooptext)
            rooptext = ""
            startflag = False
        elif "円" in text:
            ...
            browser.get(link + price)#ページ読み込み
            talk_c(price_only+ "円付近の商品です。ごらんください")
            talk_c(rooptext)
            rooptext = ""
            startflag = False
                #価格で絞り込む
        ...
        #メーカーで絞り込む
            ...
            link = AmazonUrl + MakerUrl
            browser.get(link)
            talk_c("キャドバリーから出ているチョコです。ごらんください")
            talk_c(rooptext)
            startflag = False
        elif "ブルボン" in text:
            ...
            link = AmazonUrl + MakerUrl
            browser.get(link)
            talk_c("ブルボンから出ているチョコです。ごらんください")
            talk_c(rooptext)
            startflag = False
        elif "リンツ" in text:
        ...
        #味で絞り込む
            ...
            link = chocoUrl + tasteUrl
            browser.get(link + price)
            talk_c("キャラメルチョコです。ごらんください")
            talk_c(rooptext)
            rooptext = ""
            startflag = False
        elif "味" in text:
            talk_c("ココナッツ、カカオ、グリーンティなどの種類があります。どれにいたしましょう。")
        elif "メーカー" in text:
            talk_c("ロッテや明治などがチョコをだしています、どのメーカーにいたしましょう。")
        elif "チョコ" in text:
